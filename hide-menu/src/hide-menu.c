...
{
	menubar = ui_lookup_widget(geany->main_widgets->window, "hbox_menubar");
	menu = ui_lookup_widget(geany->main_widgets->window, "menubar1");
	...
	gtk_widget_add_events(geany->main_widgets->toolbar, GDK_POINTER_MOTION_MASK | GDK_LEAVE_NOTIFY_MASK);
	gtk_widget_set_size_request(menubar, -1, height);
